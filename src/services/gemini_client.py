...
class GeminiClient:
            ...
            try:
                ...
                if status == 404 and "not found" in error_text.lower():
                    raise ValueError(
                        ...
                        f'Try: gemini-2.0-flash, or gemini-1.5-pro. '
    ...
    def _build_analysis_prompt(
        ...
        context_parts = []
        if purpose:
            context_parts.append(f"Назначение продукта: {purpose}")
        if application:
            context_parts.append(f"Способ применения: {application}")
        context = "\n".join(context_parts)

        return f"""Ты ведущий эксперт по косметической химии, биохимии и нормативному регулированию косметики (ТН ВЭД, ТР ТС 009/2011).

ЗАДАЧА:
Проанализируй текст, извлеченный из технического документа (Composition/Formula/Specification), и заполни 8 специфических форматов состава.
...
ИНСТРУКЦИИ ПО ИЗВЛЕЧЕНИЮ СОСТАВА:
1. Активные ингредиенты: Выдели те вещества, которые несут основную функциональную нагрузку (антиоксиданты, пептиды, витамины, экстракты, кислоты, увлажнители). Растворители (Water/Aqua), эмоленты (Isododecane, Glycerin), эмульгаторы и консерванты НЕ являются активными компонентами, если они не заявлены как ключевые.
   - ВАЖНО: Обязательно заполни поля "active_ru_pct" и "active_en_pct", если в документе напротив активного компонента есть буква или значение процента.
2. Полный состав: Перечисли ВСЕ ингредиенты в порядке убывания концентрации.
3. Обработка процентов (%): 
   - Если в документе указаны буквы (Montibello style), обязательно расшифровывай их в диапазоны:
     A1 (75-100%), A2 (50-75%), B (25-50%), C (10-25%), D (5-10%), E (1-5%), F (0.1-1%), G (0-0.1%).
   - В полях "с %" записывай название и диапазон в скобках. Пример: "Aqua (75-100%)", "Glycerin (10-25%)". НЕ оставляй голые буквы.
   - В полях "без %" записывай только названия ингредиентов через запятую.
4. Перевод на русский (_ru_):
   - Переводи названия экстрактов и общепринятых веществ (например: "Экстракт кокоса", "Гиалуроновая кислота", "Вода"). 
   - Сложные химические INCI названия можно оставлять на латинице или использовать общепринятую кириллицу.
5. Английский язык (_en_): Используй стандартную номенклатуру INCI.
6. ЯЗЫК ОТВЕТА:
   - Поля `product_reason`, `reg_reason`, `category`, `product_type`, `active_ru_...`, `full_ru_...` должны быть заполнены НА РУССКОМ ЯЗЫКЕ.
   - Поля `_en_` заполняй на английском/INCI.

ТРЕБОВАНИЯ К JSON:
Верни JSON с 14 полями:
{{
  "category_code": "код ТН ВЭД (3304, 3305 или 3307)",
  "category": "Название категории ТН ВЭД",
  "product_type": "Вид продукции (например: Крем для лица)",
  "product_reason": "Детальное обоснование выбора кода ТН ВЭД на основе состава и свойств",
  "reg_doc": "СГР" (если есть спец. назначение/детское/пептиды/активные кислоты) или "Декларация",
  "reg_reason": "Подробное юридическое и техническое обоснование выбора документа (напр: 'Продукт содержит пептиды и растительные экстракты с активным воздействием, что согласно ТР ТС 009/2011 требует СГР')",
  "active_ru_no_pct": "Список активных (рус, через запятую, без %)",
  "active_en_no_pct": "List of active (eng, comma separated, no %)",
  "active_ru_pct": "Список активных с % (рус, расшифрованный диапазон, напр: Экстракт кокоса (0-0.1%))",
  "active_en_pct": "List of active with % (eng, expanded range, e.g: Coconut Extract (0-0.1%))",
  "full_ru_no_pct": "Полный состав (рус, через запятую, без %)",
  "full_en_no_pct": "Full composition (eng, comma separated, no %)",
  "full_ru_pct": "Полный состав с % (рус, расшифрованные диапазоны)",
  "full_en_pct": "Full composition with % (eng, expanded ranges)"
}}
