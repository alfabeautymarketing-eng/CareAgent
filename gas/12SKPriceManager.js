/**
 * =======================================================================================
 * 12PriceManager.js — УПРАВЛЕНИЕ ЛИСТОМ ПРАЙС
 * ---------------------------------------------------------------------------------------
 * ОПИСАНИЕ:
...
(function (Lib, global) {
  ...
  Lib.addOrderItemsToPrice = function(options) {
    ...
    try {
      ...
      // Получаем данные с листа Расчет цены для заполнения Цена, RRP, E-comerc
      var calcSheetName = Lib.CONFIG && Lib.CONFIG.SHEETS && Lib.CONFIG.SHEETS.PRICE_CALCULATION;
      var calcSheet = calcSheetName ? ss.getSheetByName(calcSheetName) : null;
      var calcIdMap = {};
      var calcIdIndex = -1;
      var calcPriceOptIndex = -1;
      var calcRrpIndex = -1;
      var calcEcomercIndex = -1;

      if (calcSheet) {
        var calcData = calcSheet.getDataRange().getValues();
        if (calcData.length >= 2) {
          var calcHeaders = calcData[0];
          calcIdIndex = _findColumnIndex(calcHeaders, "ID");
          calcPriceOptIndex = _findColumnIndex(calcHeaders, "Расчетная цена Опт");
          calcRrpIndex = _findColumnIndex(calcHeaders, "РРЦ");
          calcEcomercIndex = _findColumnIndex(calcHeaders, "E-comerc");

          // Создаем карту ID -> индекс строки для быстрого поиска
          if (calcIdIndex !== -1) {
            for (var i = 1; i < calcData.length; i++) {
              var calcId = calcData[i][calcIdIndex];
              if (calcId) {
                calcIdMap[String(calcId).trim()] = i;
              }
            }
          }
        }
      ...
      var pricePriceIndex = _findColumnIndex(priceHeaders, "Цена");
      var priceRrpIndex = _findColumnIndex(priceHeaders, "RRP");
      var priceEcomercIndex = _findColumnIndex(priceHeaders, "E-comerc");
      ...
      for (var i = startRow; i < endRow; i++) {
        if (_isAddToPriceFlagSet(orderData[i][orderAddToPriceIndex])) {
          ...
          // Цена, RRP, E-comerc - из листа Расчет цены
          if (calcIdMap[orderId] !== undefined && calcData) {
            var calcRowIndex = calcIdMap[orderId];
            var calcRow = calcData[calcRowIndex];

            // Цена = Расчетная цена Опт
            if (pricePriceIndex !== -1 && calcPriceOptIndex !== -1) {
              newPriceRow[pricePriceIndex] = calcRow[calcPriceOptIndex];
            }

            // RRP = РРЦ
            if (priceRrpIndex !== -1 && calcRrpIndex !== -1) {
              newPriceRow[priceRrpIndex] = calcRow[calcRrpIndex];
            }

            // E-comerc
            if (priceEcomercIndex !== -1 && calcEcomercIndex !== -1) {
              newPriceRow[priceEcomercIndex] = calcRow[calcEcomercIndex];
            }
          }
  ...
  Lib.removeOrderItemsFromPrice = function(options) {
    ...
    try {
      ...
      // Собираем ID из листа Заказ, которые нужно удалить (где флаг = "Нет")
      var idsToRemove = {};
  ...
  var PRICE_LINE_ID_LOOKUP = (function() {
    var entries = [
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "ОЧИЩЕНИЕ И ТОНИЗАЦИЯ", 1],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "ГЛУБОКОЕ ОЧИЩЕНИЕ", 2],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "ПИЛИНГИ (пилинг системы с регулируемым PH)", 3],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "АКТИВНЫЕ КОМПЛЕКСЫ", 4],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "Активные комплексы (специального назначения)", 5],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "МАССАЖНЫЕ СРЕДСТВА", 6],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "МАСКИ", 7],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "КРЕМЫ", 8],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "Средства восстановления и защиты кожи", 9],
      ["ПРОФЕССИОНАЛЬНЫЙ УХОД", "ПРЕПАРАТЫ ДЛЯ АППАРАТНОЙ КОСМЕТОЛОГИИ", 10],
      ["СРЕДСТВА ПО УХОДУ ЗА ТЕЛОМ", "Очищение", 11],
      ["СРЕДСТВА ПО УХОДУ ЗА ТЕЛОМ", "Массаж", 12],
      ["СРЕДСТВА ПО УХОДУ ЗА ТЕЛОМ", "Обертывание", 13],
      ["СРЕДСТВА ПО УХОДУ ЗА ТЕЛОМ", "Кремы для тела", 14],
      ["СРЕДСТВА ПО УХОДУ ЗА ТЕЛОМ", "ПРЕПАРАТЫ ДЛЯ АППАРАТНОЙ КОСМЕТОЛОГИИ ДЛЯ ТЕЛА", 15],
      ["СТЕРИЛЬНЫЕ ПРЕПАРАТЫ (проф)", "Монопрепараты", 16],
      ["СТЕРИЛЬНЫЕ ПРЕПАРАТЫ (проф)", "Готовые сбалансированные коктейли", 17],
      ["ДОМАШНИЙ УХОД", "ОЧИЩЕНИЕ И ТОНИЗАЦИЯ", 18],
      ["ДОМАШНИЙ УХОД", "ГЛУБОКОЕ ОЧИЩЕНИЕ", 19],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "Увлажнение и питание, 20+", 20],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "Для жирной и комбинированной кожи, 20+", 21],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "Для жирной и комбинированной кожи, 35+", 22],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "Для жирной кожи с АКНЕ", 23],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "Для сухой и нормальной кожи, 35+", 24],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "Укрепление, редермализация, 45+", 25],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "ОСВЕТЛЯЮЩИЕ ПРЕПАРАТЫ: Борьба с пигментацией", 26],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "ОСВЕТЛЯЮЩИЕ ПРЕПАРАТЫ: Профилактика", 27],
      ["ЕЖЕДНЕВНЫЙ УХОД (Кремы)", "Восстановление, регенерация и защита кожи", 28],
      ["АКТИВНЫЕ КОМПЛЕКСЫ", "СИЯНИЕ, УПРУГОСТЬ И РЕГЕНЕРАЦИЯ", 29],
      ["АКТИВНЫЕ КОМПЛЕКСЫ", "ANTI_AGE", 30],
      ["АКТИВНЫЕ КОМПЛЕКСЫ", "ANTI_AGE, восстановление и регенерация", 31],
      ["АКТИВНЫЕ КОМПЛЕКСЫ", "Кремы для глаз, губ, шеи и декольте", 32],
      ["АКТИВНЫЕ КОМПЛЕКСЫ", "МАСКИ", 33],
      ["АКТИВНЫЕ КОМПЛЕКСЫ", "СОЛНЦЕЗАЩИТНЫЕ СРЕДСТВА", 34],
      ["АКТИВНЫЕ КОМПЛЕКСЫ", "КРЕМЫ С тонирующим эффектом", 35],
      ["УХОД ЗА ТЕЛОМ", "Очищение", 36],
      ["УХОД ЗА ТЕЛОМ", "Укрепление, Увлажнение , Питание", 37],
      ["УХОД ЗА ТЕЛОМ", "ANTICELLULITE", 38],
      ["УХОД ЗА ТЕЛОМ", "МАССАЖ И АКТИВИЗИРУЮЩИЕ ПРОЦЕДУРЫ", 39],
      ["УХОД ЗА ТЕЛОМ", "УХОД ЗА РУКАМИ", 40],
      ["УХОД ЗА ТЕЛОМ", "НАБОРЫ", 41],
      ["УХОД ЗА ВОЛОСАМИ", "", 42]
    ];
    var map = {};
