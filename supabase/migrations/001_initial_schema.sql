...
-- ============================================================================
-- 8. ФУНКЦИИ ПОИСКА И ФИЛЬТРАЦИИ
-- ============================================================================

-- Функция полнотекстового поиска по логам
CREATE OR REPLACE FUNCTION search_logs(search_query TEXT, limit_count INT DEFAULT 100)
RETURNS TABLE (
  id UUID,
  timestamp TIMESTAMP,
  level VARCHAR,
...
BEGIN
  ...
  SELECT
    ...
    logs.message,
    logs.function_name,
    ts_rank(logs.search_text, to_tsquery(search_query)) as rank
  FROM logs
  WHERE logs.search_text @@ to_tsquery(search_query)
  ORDER BY rank DESC
  LIMIT limit_count;
END;
$$ LANGUAGE plpgsql;
