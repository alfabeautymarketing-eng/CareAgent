...
Если вы хотите, чтобы PR был создан автоматически после того, как вы вставите в терминал PAT, используйте скрипт:

```bash
# Запустите из корня репозитория
./scripts/create_pr_with_token.sh
```

Как это работает:

- Скрипт попросит подтвердить действие;
- Попросит вставить PAT (ввод скрыт, токен не будет напечатан);
- Выполнит `gh auth login --with-token` и создаст PR от ветки `ci/add-deploy-workflow`;
- По окончании предложит разлогиниться (рекомендуется разлогиниться и удалить/отозвать PAT в GitHub).

Важное замечание: токен **не** хранится в репозитории и не печатается в логах; всё равно не забудьте отозвать его в GitHub, если он был временным.
